...
// Utilisation de l'API C de TensorFlow Lite pour une liaison stable
...
struct EKFState {
    ...
    float acc_bias[3] = {0.0f}, gyro_bias[3] = {0.0f}, sum_acc[3] = {0.0f}, sum_gyro[3] = {0.0f};
    const float R = 0.5f;
    const float Q = 0.01f;
};
...
void runInference() {
    if (!g_ekf.is_calibrated || !g_interpreter) return;
    ...
    for (int i = 0; i < WINDOW_SIZE; ++i) {
        ...
        for(int j=0; j<3; j++) input_data[i*10+j] = imu_buffer[idx].acc[j];
        for(int j=0; j<3; j++) input_data[i*10+3+j] = imu_buffer[idx].gyro[j];
        for(int j=0; j<4; j++) input_data[i*10+6+j] = imu_buffer[idx].ori[j];
    }

    if (TfLiteInterpreterInvoke(g_interpreter) != kTfLiteOk) return;
    ...
    rotateVectorByQuaternion(output_data[0], output_data[1], imu_buffer[last_idx].ori, dx_world, dy_world);
...
Java_com_example_geo_1slam_footslam_FootSlamManager_processPressure(JNIEnv* env, jobject thiz, jfloat p, jlong ts) {
    if (g_ekf.reference_pressure < 0) { g_ekf.reference_pressure = p; return; }
    float altitude = 44330.0f * (1.0f - pow(p / g_ekf.reference_pressure, 0.1903f));
...
Java_com_example_geo_1slam_footslam_FootSlamManager_loadModelNative(JNIEnv* env, jobject thiz, jobject am, jstring path) {
    ...
    g_model = TfLiteModelCreate(buf.data(), size);
    ...
    if (TfLiteInterpreterAllocateTensors(g_interpreter) != kTfLiteOk) return JNI_FALSE;
...
Java_com_example_geo_1slam_footslam_FootSlamManager_processAccelerometer(JNIEnv* env, jobject thiz, jfloat x, jfloat y, jfloat z, jlong ts) {
    ...
    if (g_ekf.last_timestamp_ns != 0) {
        float dt = (ts - g_ekf.last_timestamp_ns) / 1e9f;
